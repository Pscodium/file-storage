            ...
            <DialogContent className='bg-white outline-none border-none z-[9999] flex flex-col items-center justify-center'>
                ...
                {file && (
                    <div className='w-full flex px-[53px]'>
                        <input className='w-full h-6 border border-1 rounded-md' placeholder={file?.name} value={fileName} onChange={(ev) => setFileName(ev.target.value)} />
                    </div>
                )}
                <div className='flex gap-3'>
                    <DialogTrigger>
                        <Button className='rounded-md bg-gray-500 text-white hover:bg-forum-navb font-bold'>CANCELAR</Button>
                    </DialogTrigger>
